...
export default function Hero() {
  ...
  return (
    <section
      id="home"
      className="relative min-h-screen lg:h-screen flex items-center justify-center overflow-hidden w-full"
    >
      {/* Background Gradient */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        <motion.div
          ...
          initial="hidden"
          animate="visible"
          // Switched flex-col to flex-col-reverse so image is first on mobile
          className="flex flex-col-reverse lg:flex-row items-center gap-12 lg:gap-16 pt-20"
        >
          {/* Text Content */}
          <div className="flex-1 text-center lg:text-left">
            ...
            >
              Computer Engineering student, Founder of a unisex fashion brand
              and Fashion & Tech entrepreneur creating sophisticated web
            ...
            >
              <Link to="projects" spy={true} smooth={true} offset={-80} duration={500} className="w-full sm:w-auto">
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="btn-primary w-full flex items-center justify-center gap-2">
                  View Projects <FiArrowRight className="w-4 h-4" />
                </motion.button>
              </Link>
              <Link to="contact" spy={true} smooth={true} offset={-80} duration={500} className="w-full sm:w-auto">
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="btn-outline w-full flex items-center justify-center gap-2">
                  Contact Me <FiMail className="w-4 h-4" />
                </motion.button>
              </Link>
              <motion.a href="#" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="btn-outline w-full sm:w-auto flex items-center justify-center gap-2 border-accent-400 text-accent-600 dark:text-accent-400 hover:bg-accent-500 hover:text-white">
            ...
            <motion.div
              variants={itemVariants}
              className="grid grid-cols-5 sm:flex sm:flex-wrap justify-center lg:justify-start gap-3"
            >
              {[
                { icon: FiGithub, href: "https://github.com/imadara-oku", label: "GitHub" },
                { icon: FiLinkedin, href: "https://linkedin.com/in/imadaraoku", label: "LinkedIn" },
                { icon: FiTwitter, href: "https://twitter.com/imadaraoku", label: "Twitter" },
                { icon: FaWhatsapp, href: "https://wa.me/2349154464229", label: "WhatsApp" },
                { icon: FaInstagram, href: "https://instagram.com/imadaraoku", label: "Instagram" },
                { icon: FaTiktok, href: "https://tiktok.com/@imadaraoku", label: "TikTok" },
                { icon: FaFacebookF, href: "https://facebook.com/imadaraakpabio", label: "Facebook" },
                { icon: SiSnapchat, href: "https://snapchat.com/add/imadara-oku", label: "Snapchat" },
                { icon: FaThreads, href: "https://threads.net/@imadaraoku", label: "Threads" },
                { icon: FiMail, href: "mailto:imadarakpabio@gmail.com", label: "Email" },
              ].map((social, i) => (
                <motion.a
                  key={social.label}
                  href={social.href}
                  ...
                  className="w-9 h-9 md:w-10 md:h-10 rounded-xl bg-white dark:bg-navy-800 border border-primary-100 dark:border-navy-700/50 flex items-center justify-center text-navy-500 dark:text-navy-400 hover:text-primary-600 transition-all duration-300"
          ...
          <motion.div
            ...
            // Removed scale-75 to keep the picture looking as it does on desktop
          ...
          >
            <div className="relative">
              ...
              {/* Status Badges - Mirror desktop exactly on mobile */}
              <div className="block"> 
                <motion.div
                  animate={{ y: [0, -10, 0] }}
                  transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
                  className="absolute -bottom-2 -right-2 px-4 py-2 rounded-xl glass-card shadow-lg text-xs sm:text-sm font-semibold text-primary-700 dark:text-primary-300 border border-white/20 whitespace-nowrap"
                >
                  🎓 Engineering Student
                </motion.div>
                <motion.div
                  ...
                  className="absolute -top-2 -left-2 px-4 py-2 rounded-xl glass-card shadow-lg text-xs sm:text-sm font-semibold text-accent-600 dark:text-accent-400 border border-white/20 whitespace-nowrap"
                >
                  👔 CEO
                </motion.div>
              </div>
